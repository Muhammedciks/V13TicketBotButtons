...
console.log(chalk.red('=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+='))
  console.log(chalk.cyan(`Made by DNA#1466`))
  console.log(chalk`{yellowBright Credits } | {blueBright DNA} | {cyanBright https://discord.gg/YXXGUV6KvY}`)
...
.setURL(`https://discord.gg/YXXGUV6KvY`)
...
async function checktoken(token){
  if (!token) {
    ...
    if(token.length != "ODk0ODUxMTY2MDA4MTI3NDk5.YVwBFw.DfPq3Av34K29X7YDbWe0TWjryno".length) {
...
.addField(`My Source Code`, `[Click Me!](https://facebook.com/dongocanh19)`, false)
      .setTimestamp()
      .setColor(`#ffffff`)
      .setFooter(`All rights belong to https://discord.gg/YXXGUV6KvY`)
  ...
  if (command == prefix + 'open') {
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      ...
      setTimeout(async () => {
        try {
          msg.delete()
          channel.send({ embed: { description: `Ticket opened by <@!${message.author.id}>`, color: `#ffffff` } })
  ...
  if (command == prefix + 'rename' || command == prefix + 'setname') {
    ...
    if (await db.get(`ticket_${channel.id}_${message.guild.id}`)) {
      let args = message.content.split(' ').slice(1).join(' ');
      if (!args) return message.reply({ embed: { description: `Please select the name you want for the ticket`, color: `#ffffff` } })
